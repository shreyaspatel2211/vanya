...
<div class="page bg-[#F9FAF9] ">
    ...
    <div class="pt-[123px] bg-[url('./dist/assets/images/about-bg.png')] bg-no-repeat bg-cover py-10 relative overflow-hidden">
        <div class="container py-[50px] lg:py-[150px] flex flex-col-reverse md:flex-row justify-between items-center">
            ...
            <div class="w-full text-white relative  max-w-[600px] mt-5 md:mt-0 banner-header">
                <h1 class="text-[#050509] text-[32px] md:text-[48px] DMSans-bold mb-7">{!! setting('get-quote.banner_title') !!}</h1>
                <p class="text-[#333] relative mb-12 text-base">{!! setting('get-quote.banner_description') !!}</p>
                </a>
            </div>
        </div>
    </div>
    <!-- banner section -->
    <div class="bg-theme1 p-6 lg:p-8">
        <div class="container">
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-x-10 gap-y-5">
                @foreach($features as $feature)
                    <div class="flex items-center gap-5">
                        <img src="./dist/assets/images/check-mark.svg" alt="">
                        <span class="text-xl lg:text-2xl text-white DMSans-bold">{{ $feature->title }}</span>
                    </div>
                @endforeach
                {{-- <div class="flex items-center gap-5">
                    <img src="./dist/assets/images/check-mark.svg" alt="">
                    <span class="text-xl lg:text-2xl text-white DMSans-bold">Accident & Illness Coverage</span>
                </div>
                <div class="flex items-center gap-5">
                    <img src="./dist/assets/images/check-mark.svg" alt="">
                    <span class="text-xl lg:text-2xl text-white DMSans-bold">Accident & Illness Coverage</span>
    ...
    <div class="py-10">
        <div class="container">
            <p class="text-theme2 text-lg lg:text-2xl mb-5">{!! setting('get-quote.long_description') !!}</p>
        ...
        <div class="container">
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-5">
                    ...
                    <div class="bg-white rounded-lg p-5 lg:p-10">
                        ...
                        <form action="{{ route('get-quote.submit') }}" method="POST">
                            ...
                            <div class="form-group mb-7">
                                <label class="text-[#252B42] text-sm DMSans-semibold mb-2 block">Name *</label>
                                <input class="w-full px-7 py-4 bg-[#F9F9F9] rounded-md text-[#9F9F9F] outline-none border border-[#E6E6E6]" type="text" placeholder="Enter Name of your pet *" name="name" required>
                                @error('name') <p class="text-red-500">{{ $message }}</p> @enderror
                            </div>

                            <div class="form-group mb-7">
                                <label class="text-[#252B42] text-sm DMSans-semibold mb-2 block">Email *</label>
                                <input class="w-full px-7 py-4 bg-[#F9F9F9] rounded-md text-[#9F9F9F] outline-none border border-[#E6E6E6]" type="text" placeholder="Enter your email *" name="email" required>
                                @error('email') <p class="text-red-500">{{ $message }}</p> @enderror
                            </div>
                        
                            <div class="form-group mb-7">
                                <label class="text-[#252B42] text-sm DMSans-semibold mb-2 block">Category *</label>
                                <div class="grid grid-cols-1 lg:grid-cols-3 gap-3 lg:gap-6">
                                    {{-- <label class="cursor-pointer bg-[#F9F9F9] rounded-lg border border-[#E6E6E6]">
                                        <input type="radio" name="category" value="dog" class="peer hidden" checked>
                                        <div class="border-2 border-transparent peer-checked:border-[#184175] rounded-lg p-14 text-center">
                                            <img src="./dist/assets/images/dog-insurance1.svg" alt="Dog" class="w-24 h-24 mx-auto mb-8 rounded-lg">
                                            <p class="mt-2 text-base text-[#252B42] DMSans-bold">Dog</p>
                                        </div>
                                    </label>
                                    <label class="cursor-pointer bg-[#F9F9F9] rounded-lg border border-[#E6E6E6]">
                                        <input type="radio" name="category" value="cat" class="peer hidden">
                                        <div class="border-2 border-transparent peer-checked:border-[#184175] rounded-lg p-14 text-center">
                                            <img src="./dist/assets/images/cat-insurance1.svg" alt="Cat" class="w-24 h-24 mx-auto mb-8 rounded-lg">
                                            <p class="mt-2 text-base text-[#252B42] DMSans-bold">Cat</p>
                                        </div>
                                    </label>
                                    <label class="cursor-pointer bg-[#F9F9F9] rounded-lg border border-[#E6E6E6]">
                                        <input type="radio" name="category" value="companion_pet" class="peer hidden">
                                        <div class="border-2 border-transparent peer-checked:border-[#184175] rounded-lg p-14 text-center">
                                            <img src="./dist/assets/images/companionpet-Insurance1.svg" alt="Companion Pet" class="w-24 h-24 mx-auto mb-8 rounded-lg">
                                            <p class="mt-2 text-base text-[#252B42] DMSans-bold">Companion pet</p>
                                        </div>
                                    </label> --}}
                                    @foreach (['dog' => 'Dog', 'cat' => 'Cat', 'companion_pet' => 'Companion Pet'] as $value => $label)
                                        <label class="cursor-pointer bg-[#F9F9F9] rounded-lg border border-[#E6E6E6]">
                                            <input type="radio" name="category" value="{{ $value }}" class="peer hidden">
                                            <div class="border-2 border-transparent peer-checked:border-[#184175] rounded-lg p-10 text-center">
                                                <img src="./dist/assets/images/{{ $value }}-insurance1.svg" alt="{{ $label }}" class="w-24 h-24 mx-auto mb-8 rounded-lg">
                                                <p class="mt-2 text-base text-[#252B42] DMSans-bold">{{ $label }}</p>
                                            </div>
                                        </label>
                                    @endforeach
                            ...
                            <div class="form-group mb-7">
                                <label class="text-[#252B42] text-sm DMSans-semibold mb-2 block">Breed *</label>
                                <select id="breedDropdown" class="w-full px-7 py-4 bg-[#F9F9F9] rounded-md text-[#9F9F9F] outline-none border border-[#E6E6E6]" name="breed" required>
                            ...
                            <div class="form-group mb-7">
                                <label class="text-[#252B42] text-sm DMSans-semibold mb-2 block">Age *</label>
                                <select class="w-full px-7 py-4 bg-[#F9F9F9] rounded-md text-[#9F9F9F] outline-none border border-[#E6E6E6]" name="age" required>
                ...
                <div class="flex items-center">
                    <img src="./dist/assets/images/cat-insurance-c.png" class="w-full max-w-[600px] ml-auto" alt="">
                </div>
            </div>
        </div>
    </div>
    <div class="py-10">
        <div class="container">
            <p class="text-theme2 text-lg lg:text-2xl mb-5">{!! setting('get-quote.description') !!}</p>
            {{-- <p class="text-theme2 text-lg lg:text-2xl mb-5">Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.</p> --}}
        </div>
    </div>
    <div class="bg-[#DFDFDF]">
...
<script>
    $(document).ready(function () {
        let defaultCategory = "dog";

        $('input[name="category"][value="' + defaultCategory + '"]').prop("checked", true);

        // Function to fetch breeds via AJAX
        function fetchBreeds(category) {
            $.ajax({
                url: "{{ route('get.breed') }}", // Replace with your actual route
                type: "GET",
                data: { category: category },
                success: function (response) {
                    let breedDropdown = $("#breedDropdown");
                    breedDropdown.empty();
                    breedDropdown.append('<option value="">Select Breed</option>'); // Default option

                    $.each(response.data, function (index, breed) {
                        breedDropdown.append(`<option value="${breed.id}">${breed.name}</option>`);
                    });
                },
                error: function () {
        ...
        // Call AJAX on page load with default "dog" category
        fetchBreeds(defaultCategory);

        // Call AJAX when a radio button is changed
        $('input[name="category"]').change(function () {
            fetchBreeds($(this).val());
        });
    });
